const { ProducerStream } = require('kafka-node'),
  ...
  Lbucket = require('./../../storage/lbucket'),
...
module.exports = class Batch extends Builder.Flow {
  constructor(options, lambda, aggrigator = {}) {
    ...
    this._storeOptions = _.pick(options, ["number", "groupBy", "attributes", "timeout", "type"]);
    this._lambda = lambda;
  ...
  async _process(batchData) {
    let data, error;
    if ($(batchData)(error)) {
      error = batchData.error
    }
    try {
      if (_.isArray(batchData.result)) {
        data = await this._processGroup(batchData.result);
      } else if (batchData instanceof Lbucket) {
        data = await this._processGroup(batchData);
      }
      else {
        data = await Promise.all(_.values(batchData.result).map(batchData => this._processGroup(batchData)));
      }
    } catch (err) {
      error = err;
    } finally {
  ...
  async _processGroup(groupData) {
    let result = await this._aggrigation(groupData, this._lambda, _.clone(this._aggrigator));
  ...
  _buildDownStream() {
    ...
    return new Klass({
      ...
      write(data, encoding, callback) {
        this.push({ data, error: data.error });
        callback();
      },
      read(size) {
      }
