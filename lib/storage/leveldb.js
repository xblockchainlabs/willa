...
const getDbPath = paths => path.join(paths, 'willadb');
const leveDBoptions = {
  keyEncoding: 'binary',
  valueEncoding: 'json'
};
...
module.exports = class LevelDb {
  constructor(options) {
    ...
    this._path = (!!fs.pathExistsSync(options.path.trim())) ? options.path.trim() : '/tmp';
    this._timerRef;
    this._active = true;
    this._error = null;
    ...
    let newbuckt = cuid();
    this.db.put('_buckts', { ids: [newbuckt] });
    this._bucktsRefs.set(newbuckt, new LBucket({ bucktId: newbuckt, groups: this._groupBy, path: this._path }));
  ...
  _getLastBucket() {
    ...
    return this.db.get('_buckts')
      ...
      .then(buckts => this.db.put('_buckts', buckts))
      .then(() => bckt)
      .catch(err => console.log(err));
  }
  ...
  _moveBucket() {
    ...
    this.db.get('_buckts')
      .then(buckts => {
        ...
        return this.db.put('_buckts', buckts);
      })
      .then(() => {
        this._bucktsRefs.set(bucktId, new LBucket({ bucktId, groups: this._groupBy, path: this._path }));
